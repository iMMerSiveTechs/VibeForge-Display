...
struct SidebarNavigation: View {
    ...
    private func badgeView(for tab: AppState.SidebarTab) -> some View {
        ...
        case .quickSetup:
            EmptyView()
        case .routes:
            let activeRoutes = appState.routeService.activeRouteIDs.count
            if activeRoutes > 0 {
                countBadge(activeRoutes, color: VFTheme.Colors.success)
            } else {
                let totalRoutes = appState.routeService.routes.count
                if totalRoutes > 0 {
                    countBadge(totalRoutes, color: VFTheme.Colors.textTertiary)
                }
            }
        default:
            EmptyView()
