...
struct MenuBarView: View {
    ...
    private var routesSummary: some View {
        let total = appState.routeService.routes.count
        let active = appState.routeService.activeRouteIDs.count
        let totalClients = appState.routeService.clients.values.reduce(0) { $0 + $1.count }
            ...
            if totalClients > 0 {
                HStack(spacing: VFTheme.Spacing.xs) {
                    Image(systemName: "ipad.and.arrow.forward")
                        .font(.system(size: 10))
                        .foregroundStyle(VFTheme.Colors.accent)
                    Text("\(totalClients) device(s) connected")
                        .font(VFTheme.Typography.caption)
                        .foregroundStyle(VFTheme.Colors.accent)
                }
            }

            ForEach(appState.routeService.routes.prefix(3)) { route in
                HStack(spacing: VFTheme.Spacing.xs) {
                    Circle()
                        .fill(appState.routeService.isActive(route.id)
                              ? VFTheme.Colors.success : VFTheme.Colors.textTertiary)
                        .frame(width: 5, height: 5)
                    Text(route.name)
                        .font(VFTheme.Typography.caption)
                        .foregroundStyle(VFTheme.Colors.textSecondary)
                    Spacer()
                    if appState.routeService.isActive(route.id) {
                        Text(":\(route.port)")
                            .font(VFTheme.Typography.mono)
                            .foregroundStyle(VFTheme.Colors.textTertiary)
                    }
